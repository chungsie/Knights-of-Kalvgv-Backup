...
#NPC deck
        ...
        class Npc
            ...
            def set_locations loc
                @xx = rand($game.scene.size_w).clamp(1,$game.scene.size_w - 1)
                @yy = rand($game.scene.size_h).clamp(1,$game.scene.size_h - 1)
                if $game.scene.characters[@xx][@yy] != $game.scene.samples[:floors]
                    set_locations(loc)
                end
            ...
            def set_jump
                ...
                if $game.scene.characters[@x + @difx][@y +  @dify] == $game.scene.samples[:floors]
                    walk(@difx+@x,@dify+@y)
                else
                    set_jump()
                end
            ...
            def walk (target_x, target_y)
                ...
                for a in (0..@tile_learning_index-1)
                    ...
                    if $game.scene.characters[@x + @difx][@y] == $game.scene.samples[:floors] 
                    @x += @difx
                    #puts "walked x"
                    else
                        @set += 1
                    ...
                    if $game.scene.characters[@x ][@y + @dify] == $game.scene.samples[:floors] 
                        @y += @dify
                        #puts "walked y"
                    else
                        @set += 1
            ...
            def turn_page
                ...
                if @page > 1 && $game.scene.npc_count != 0 && (@chapter == 3)
                    @page -= 1
                    @trigger = false
                end
                if $game.scene.habit_count > 0 && @chapter == 3
                    @chapter = 4
                    @page = 0
                    @trigger = false
                end
                ...
                @page += 1 if $game.scene.in_dialogue && @trigger
        ...
        class Room 
            ...
            attr :tunnel_cards, :tunnel_count, :tunnel_points, :tunnel_card_select
        
            def initialize count
                #@npcs = (50 + rand(50)).map { Npc.new }
                @size_w = [8,10,12,14,16].sample
                @size_h = [8,10,12,14,16].sample
                @samples = { wall: 6, floors: 9, hero_place: 0, guide_p: 7, elf_statue: 10, dwarf_statue: 11, token: 12, chest: 13, selection: 14, sign_post: 17 }
            ...
            def check_for_wall char
                return true if @characters[char.x+1][char.y] == @samples[:wall] || @characters[char.x-1][char.y] == @samples[:wall] || @characters[char.x][char.y+1] == @samples[:wall] || @characters[char.x][char.y-1] == @samples[:wall]
                return false
            end
        
            def get_wall_location char
                @ret = []
                return @ret = [char.x+1, char.y] if @room[char.x+1][char.y] == @tiles[:wall] 
                return @ret = [char.x-1, char.y] if @room[char.x-1][char.y] == @tiles[:wall] 
                return @ret = [char.x, char.y+1] if @room[char.x][char.y+1] == @tiles[:wall] 
                return @ret = [char.x, char.y-1] if @room[char.x][char.y-1] == @tiles[:wall]
                puts @ret.to_s + " wall location"
            end
            def get_wall_direction char
                @ret = []
                return 1 if @room[char.x+1][char.y] == @tiles[:wall] 
                return -1 if @room[char.x-1][char.y] == @tiles[:wall] 
                return 2 [char.x, char.y+1] if @room[char.x][char.y+1] == @tiles[:wall] 
                return -2 [char.x, char.y-1] if @room[char.x][char.y-1] == @tiles[:wall]
                puts @ret.to_s + " wall location"
            end
        
            def spawn_tunnel coords
                if coords != nil
                    @room[coords[0]][coords[1]] = @tiles[:floors]
                    @temp = @tunnel_card_select
                    @temp_x = @hero.x
                    @temp_y = @hero.y
                    @room[coords[0]][coords[1]] = @tiles[:water]
                    @dir_temp = get_wall_direction(@hero)
                    @tunnel_points[coords[0]][coords[1]] = 1
                    $game.tunnels << Tunnel.new(@temp.image, coords[0], coords[1], @hero, @dir_temp, @guide)
                    $game.tunnels[$game.tunnel_count].tunnel_connections[0] = coords
                    $game.tunnels[$game.tunnel_count].size_h  = @temp.length[@temp.image]
                    $game.tunnels[$game.tunnel_count].size_w  = @temp.width[@temp.image]
                    $game.tunnels[$game.tunnel_count].turn = @temp.turn[@temp.image]
                    $game.tunnels[$game.tunnel_count].guide = @guide
                    $game.tunnels[$game.tunnel_count].room_number = $game.room_count + 1
                    $game.room_count += 1
                    $game.tunnel_count += 1
                    @hero.x = @temp_x 
                    @hero.y = @temp_y
            ...
            def clicks()
                for y in (0..@tunnel_count)
                    if inputs.mouse.point.inside_rect?([450 + (y+1 * 100), 575,75,125]) && @tunnel_count > 0
                        #puts "this is a tunnel card"
                        puts check_for_wall(@hero)
                        @guide.text = "PLAY TUNNEL CARD? PRESS E" if check_for_wall(@hero)
                        @guide.text = "THIS IS A TUNNEL CARD, STAND NEXT TO A WALL TO PLAY" if !check_for_wall(@hero)
                        if @guide.text == "PLAY TUNNEL CARD? PRESS E"
                            @guide.text = "TUNNEL CARD PLAYED"
                            @choice = "tunnel"
                            @tunnel_card_select = @tunnel_cards[y]
                            puts "click on tunnel"
                            @tunnel_cards.delete_at(y)
                            @tunnel_count -= 1
                        end
                    end
                end
                ...
                if @choice == "tunnel"
                    @coords = []
                    @coords = get_wall_location(@hero)
                    puts @coords.to_s + " wall cords"
                    spawn_tunnel(@coords)
                    @choice = "nothing"
                end
            ...
            def draw_tunnel_card
                ...
                puts @tunnel_cards[@tunnel_count].deck[@tunnel_cards[@tunnel_count].image].to_s + "card"
                @tunnel_count += 1
            end
            ...
            def apply_damage defe
                ...
                apply_damage(defe) if defe.red < 3 || defe.race == :noone
                return false
            end
            ...
            def input args
                $game.tunnels.each do
                    |check|
                    if @hero.x == check.tunnel_connections[0][0] && @hero.y = check.tunnel_connections[0][1]
                        @guide.text = "PRESS E TO ENTER THE TUNNEL"
                        if inputs.keyboard.key_down.e 
                            puts "pressed e"
                            puts check
                            transfer_to_tunnel(check)
                        end
                    end
                end
                ...
                #puts $timer.elapsed?
                if @hazards > 0 && @room[@hero.x][@hero.y] == @tiles[:lava] && $timer.elapsed? && @choice != "execute jump"
                    puts "ouch" + @hero.red.to_s + " number of 0 stats"
                    @hero.stats[apply_damage(@hero)] -= 1 and @hero.health -= 1 if @hero.red < 10 && @hero.race != :noone
                ...
                if @choice != "execute jump" && @choice != "execute dash" && @choice != "dash"
                    if inputs.keyboard.key_down.left
                        if @characters[@hero.x - 1][@hero.y] == @samples[:floors] || @characters[@hero.x - 1][@hero.y] == @samples[:sign_post]
                            @characters[@hero.x][@hero.y] = @samples[:floors]
                            @hero.x -= 1
                            @trigger = true
                    ...
                    if inputs.keyboard.key_down.right
                        if @hero.x < @size_w - 1
                            if @characters[@hero.x + 1][@hero.y] == @samples[:floors] || @room[@hero.x + 1][@hero.y] == @tiles[:water]
                                @characters[@hero.x][@hero.y] = @samples[:floors]
                                @hero.x += 1
                                @trigger = true
                    ...
                    if inputs.keyboard.key_down.down
                        if @characters[@hero.x][@hero.y + 1] == @samples[:floors] || @characters[@hero.x][@hero.y + 1] == @samples[:sign_post]
                            @characters[@hero.x][@hero.y] = @samples[:floors]
                            @hero.y += 1
                            @trigger = true
                            $game.play_foot_steps(args) if $game.args.audio[:footsteps] == nil
            ...
            def get_samples
                return @samples[:floors] || @samples[:sign_post]
            end
        ...
        class Tunnel < Room
            attr_gtk
            attr :pos_tunnel_x, :pos_tunnel_y, :tunnel_id, :room, :tile_w, :tile_h, :size_w, :size_h, :hero, :dir, :dir_directory
            attr :samples, :tiles, :tunnel_connections, :turn, :npc_count, :npc_deck_drawn, :choice
        
            def initialize type, x, y, character, direction, guide
                @samples = { wall: 6, floors: 9, hero_place: 0, guide_p: 7, elf_statue: 10, dwarf_statue: 11, token: 12, chest: 13, selection: 14 }
                @tiles = { wall: 0, floors: 1, water: 2, lava: 3 }
                @deck = { shoot: 0, turn_right: 1, turn_left: 2 }
                @length = { shoot: 8, turn_right: 4, turn_left: 4 }
                @width = { shoot: 4, turn_right: 8, turn_left: 8 }
                @turn = { shoot: 0, turn_right: 1, turn_left: -1 }
                @dir_directory = { up: 2, down: -2, right: 1, left: -1 }
                @tunnel_connections = Array.new(2) { Array.new(2,0) }
                @tunnel_id = 0
                @choice = "nothing"
                @pos_tunnel_x = 1280/2
                @pos_tunnel_y = (700/2) + 200
                @tile_w = 32
                @tile_h = 32
                @size_w = @width[type]
                @size_h = @length[type]
                @guide = guide
                @hero = character
                @room = Array.new(@size_w) { Array.new(@size_h, 9) }
                @dir = direction
                @npc_deck_drawn = []
                @npc_count = 0
                @characters = Array.new(@size_w) { Array.new(@size_h, 9) }
                ...
                for i in (0...@size_w) 
                    for j in (0...@size_h)
                        if @dir == @dir_directory[:up] || @dir == @dir_directory[:down]
                            if i == 0 || i == @size_w
                                @room[i][j] = @tiles[:wall]
                            else
                                @room[i][j] = @tiles[:floors]
                        ...
                        elsif @dir == @dir_directory[:left] || @dir == @dir_directory[:right]
                            if j == 0 || j == @size_h
                                @room[i][j] = @tiles[:wall]
                            else
                                @room[i][j] = @tiles[:floors]
                ...
                @characters[@hero.x][@hero.y] = @samples[:hero_place]
                @characters[@guide.x][@guide.y] = @samples[:guide_p]
                puts @characters.to_s + "characters"
                puts @room
            end
        ...
        class DM
            ...
            attr :time_redux, :picked_s, :tunnels, :tunnel_connections, :tunnel_count, :scene, :room_count
            ...
            def check_healing_near
                if @scene.habit_count > 0 && $timer.elapsed?
                    for a in (0..@scene.habit_count - 1)
                        #puts a, @scene.habitations[a].x
                        if @scene.distance_habitat(@scene.habitations[a].x,@scene.habitations[a].y)
                            heal_player(@scene.hero) if @scene.hero.max_health != @scene.hero.health
                        end
                    end
                end
            end
            ...
            def tick args
                $timer ||= 0
                check_healing_near() if @scene.habit_count > 0
                @scene.input(args)
                $timer = state.tick_count + (60 / @time_redux) if $timer < state.tick_count 
                if @scene.hero.line_of_sight == 3 && @scene.hero.race == :elf || @scene.hero.race == :dwarf
                    @scene.hero.line_of_sight = 5
                end
                for i in (0...@scene.npc_count)
                    #puts $time_now
                    #@scene.npc_count -= 1; exit if @scene.npc_deck_drawn[i] == nil
                    if  @scene.npc_deck_drawn[i] != nil && @scene.npc_count > 0 && @scene.npc_deck_drawn[i].names != "guide" && @scene.npc_deck_drawn[i].decision_timer < state.tick_count && @scene.npc_deck_drawn[i].names != "guide"
                        @scene.ai(@scene.npc_deck_drawn[i])
                        @scene.npc_deck_drawn[i].decision_timer = state.tick_count + 45
                        if @scene.npc_deck_drawn[i].initiate 
                            play_ranged(args)
                        end
                        #puts @scene.npc_deck_drawn[i].decision_timer.to_s + " count to"
                    end
                    if @scene.npc_deck_drawn[i].initiate == true && @scene.npc_deck_drawn[i].names != "guide"
                        @scene.timer(@scene.npc_deck_drawn[i].decision_timer , @scene.npc_deck_drawn[i])
                    end
                end
        
                for a in (0...@scene.size_w)
                    for b in (0...@scene.size_h)
                        outputs.sprites << @scene.world(a,b) if @scene.distance(a,b) #[@pos_room_x + (a * 16), @pos_room_y - (b* 16), @tile_w, @tile_h, 'sprites/world.png', @tiles[@room[a][b]]*16,0,@tile_w,@tile_h]
                        outputs.sprites << @scene.character_render(a,b) if @scene.distance(a,b)#[@pos_room_x + (a * 16), @pos_room_y - (b* 16), @characters[a][b]]
                    end
                end
                outputs.sprites << @scene.ui_left_bg()
                for j in (0...@scene.habit_count)
                    outputs.sprites << @scene.draw_habitation(j) if @scene.habit_count > 0 && @scene.distance(@scene.habitations[j].x,@scene.habitations[j].y)
                end
                for k in (0...@scene.loot_count)
                    outputs.sprites << @scene.draw_loot(@scene.loot[k - 1])
                end
                    #puts j
                    if @state_of_menu_left == [:default]
                        outputs.sprites << @scene.ui_button_atk()
                        outputs.sprites << @scene.ui_button_skill()
                        outputs.sprites << @scene.ui_button_spell()
                        outputs.sprites << @scene.ui_stats()
                    elsif @state_of_menu_left == [:skills] || @state_of_menu_left == [:spells] || @state_of_menu_left == [:stats] || state_of_menu_left == [:magic_skills]
                        outputs.sprites << @scene.ui_back()
                    end
                if @state_of_menu_left == [:skills]
                    outputs.sprites << @scene.ui_button_int()
                    outputs.sprites << @scene.ui_button_lock()
                    outputs.sprites << @scene.ui_button_jump()
                    outputs.sprites << @scene.ui_button_dash()
                    outputs.sprites << @scene.ui_button_magic()
                end
                if @state_of_menu_left == [:magic_skills]
                    outputs.sprites << @scene.ui_button_stealth()
                    outputs.sprites << @scene.ui_button_heal()
                end
                if @state_of_menu_left == [:stats]
                    outputs.labels << [80, 520, "STAT BLOCK"]
                    outputs.labels << [80, 500, @scene.hero.stats[:melee].to_s + " M"]
                    outputs.labels << [80, 480, @scene.hero.stats[:ranged].to_s + " R"]
                    outputs.labels << [80, 460, @scene.hero.stats[:touch].to_s + " TM"]
                    outputs.labels << [80, 440, @scene.hero.stats[:ranged_magic].to_s + " RM"]
                    outputs.labels << [88,420, "SKILLS_____________"]
                    outputs.labels << [88,400, @scene.hero.skills[:thievery].to_s + " THIEF"]
                    outputs.labels << [88, 380, @scene.hero.skills[:stealth].to_s + " STEALTH"]
                    outputs.labels << [88, 360, @scene.hero.skills[:healing].to_s + " HEALING"]
                    outputs.labels << [88, 340, @scene.hero.skills[:mend].to_s + " MENDING"]
                    outputs.labels << [88, 320, @scene.hero.skills[:lockpicking].to_s + " PICKLOCKS"]
                    outputs.labels << [88, 300, @scene.hero.skills[:intimidate].to_s + " INTIMIDATE"]
                    outputs.labels << [88, 280, @scene.hero.skills[:jump].to_s + " JUMP"]
                    outputs.labels << [88, 260, @scene.hero.skills[:dash].to_s + " DASH"]
                end
                outputs.labels << [80,600, @scene.hero.health.to_s + " HP"]
                outputs.labels << [160,600, @scene.hero.race]#s[@scene.hero.race]]
                outputs.labels << [160,620, @scene.hero.klass]
                outputs.sprites << @scene.ui_top_bg()
                outputs.sprites << @scene.ui_weapon_cards()
                ...
                outputs.labels << [350, 555, @scene.guide.text]
                @scene.clicks()
                @scene.update() if inputs.keyboard.key_down.raw_key
                outputs.sprites << ui_select_jump() if @scene.choice == "jump"
                if @scene != nil
                    count = 0
                    @scene.tunnel_cards.each do
                        |c| 
                        outputs.sprites << ui_tunnel_card(c, count)
                        count += 1
                    end
                end
                if @scene.choice == "attacked"
                    if @scene.hero.default_weapon == 0
                        play_punch(args)
                    end
                    if @scene.hero.default_weapon == 1
                        play_melee(args)
                    end
                    if @scene.hero.default_weapon == 2
                        play_ranged(args)
                    end
                    @scene.choice = "nothing"
                end
                if @picked_s == true
                    play_lock_open(args)
                    @picked_s = false
        ...
        def tick args
            ...
            if $game != nil && $game.state_of_game != :game_over
                $game.args = args and $game.scene.args = args 
                $game.args.state = args.state and $game.tick(args)
            ...
            elsif $game != nil && $game.state_of_game == :game_over
